...
type 立管管夹Drawer() =
    // 实例的唯一标识
    let getKey (data:立管管夹) = sprintf "%f" data.Dw

    let precalc (data:立管管夹) =
    ...
    let side (data:立管管夹) =
        ...
        //螺栓孔中心
        let c1 = circle(new Point2d(0.5*data.C, data.B - data.B1)     , 0.5*data.M1 + 1.):>Entity
        let c2 = circle(new Point2d(0.5*data.D, data.B - data.M2 - 2.), 0.5*data.M2 + 1.):>Entity
        let c3 = circle(new Point2d(0.5*data.D, data.M2 + 2.)         , 0.5*data.M2 + 1.):>Entity

        let c4 = c3 |> mirror vAxis
        let c5 = c2 |> mirror vAxis
        let c6 = c1 |> mirror vAxis
    ...
    [<CommandMethod("立管管夹")>]
    member this.Drawer() =
        let objects = Lake.Clasps.立管管夹.DataRecords
