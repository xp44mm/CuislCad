...
module mod突面法兰盖 =
    let getKey (data:突面法兰盖) = sprintf "%f,%f" data.PN data.DN

    let front (data:突面法兰盖) =
        let phi = 
            Lake.Valves.法兰螺纹孔.DataRecords
            |> Seq.find(fun h -> h.M = data.M)
            |> fun h -> h.Phi
...
type 突面法兰盖Drawer() =
    [<CommandMethod("突面法兰盖")>]
    member this.Drawer() =
        let objects = Lake.Valves.突面法兰盖.DataRecords

        let views = ["前面",mod突面法兰盖.front;"侧面",mod突面法兰盖.side]

        Part.tryGetBlockDef objects views mod突面法兰盖.getKey
        |>  function
            |Some(blockId,view) ->
                let db,ed = 
